...
8. After reconciliation, if any hard forced exit is **still held at the broker** (both the main turn and `force_sell` failed to fill), a `WARNING` is printed and the alert re-fires next cycle — manual sell may be required.
...
Computes the 4-line ribbon — **blue=EMA(8), green=TEMA(13), yellow=TEMA(21), red=TEMA(55)** — from real price data, matching the TradingView chart the strategy is read from ("TEMA 13 21 55" + "EMA 8"). TEMA = triple EMA (`3*e1 − 3*e2 + e3`), much lower lag than a plain EMA; a plain-EMA 55 line reads BUY long after the chart reads SELL (this held CLOV through a 2-day fade the chart flagged SELL at breakeven on 2026-06-11). Lengths are always **8/13/21/55 BARS** at the active chart interval (`SIGNAL_INTERVAL`, default `1h`; `run.sh` exports `30m`): on a 30m chart, 55 bars = 55 half-hours. The lengths never change — only the bar interval does. Data sources tried in order: (1) `data/<SYMBOL>.csv` local file, (2) Yahoo Finance API for all intervals. Returns `INSUFFICIENT_DATA` if `< 60 bars` — the agent blocks on this, never fabricates a signal.
...
Signal classification: `red < min(others)` = BUY, `red > max(others)` = SELL, else NEUTRAL. `WARMUP_OK` is 165 bars (TEMA's triple cascade carries seed bias ~3× longer than a plain EMA); below that the signal still emits with a "not fully seeded" note.
...
- **Ribbon signal gate** (TEMA 13/21/55 + EMA 8): the red(55) `ENTER_LONG` transition triggers buys. Sells are **state-based for held positions**: `SELL` state (red on top) on a symbol we own sells immediately, whether or not this bar has the `EXIT` transition — the edge may have passed on a bar the bot never evaluated. `HOLD` = stay in position (already in BUY zone); `NO_ACTION` on an unheld symbol = do nothing. Weekend picks in `HOLD` state are still valid buy targets during execution.
